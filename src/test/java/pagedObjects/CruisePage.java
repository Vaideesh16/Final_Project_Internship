...
public class CruisePage {
        ...
        for (int i = 0; i < cruises.size() && i < top; i++) {
            ...
            for (String w : driver.getWindowHandles()) {
                ...
                if (!w.equals(cur)) {
                    ...
                    try {
                        // Finding the duration inside the card
                    ...
                    // Read languages inside the card
                    ...
                    if (langs.isEmpty()) {
                        ...
                        // Printing the languages to console
                    ...
                    driver.close();
                    // switches to the main window which displays cruise cards
    ...
    public List<String[]> getCruiseData(JavascriptExecutor js) {
        ...
        int top = 3;
        log.info("Extracting data for top " + top + " cruises");
        String cur = driver.getWindowHandle();

        // default values for price, duration and languages
        for (int i = 0; i < cruises.size() && i < top; i++) {
            String title = cruises.get(i).findElement(By.xpath(".//a")).getText();
            ...
            try {
                price = cruises.get(i).findElement(By.xpath(".//div[contains(@class,'e7addce19e')]")).getText();
            } catch (Exception e) {
            ...
            log.info("Cruise Extracted — Name: " + title + " | Price: " + price + " | Duration: " + duration + " | Languages: " + languages);
            results.add(new String[]{title, price, duration, languages});
        }

        log.info("Total cruises extracted: " + results.size());
        return results;
    }
}
